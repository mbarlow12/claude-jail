#!/usr/bin/env bash
# claude-jail installer
# Usage: curl -fsSL https://raw.githubusercontent.com/mbarlow12/claude-jail/main/install.sh | bash
#
...
get_rc_file() {
    ...
    case "$shell_env" in
        omz|zsh)
            ...
            ;;
        *)
            # On macOS, .bash_profile is more common than .bashrc
    ...
    if [[ ${#missing[@]} -gt 0 ]]; then
        ...
        case "$os" in
            ...
            macos)
                ...
                echo ""
                echo "Note: bubblewrap on macOS has limited functionality compared to Linux."
                echo "Some isolation features may not work as expected."
                ;;
        esac
...
main() {
    ...
    case "$os" in
        windows)
            ...
            ;;
        unknown)
            warn "Unknown operating system. Proceeding anyway..."
            ;;
        macos)
            warn "macOS support is experimental. Some isolation features may be limited."
            ;;
    esac
    ...
    # Shell configuration
    local rc_file
    rc_file=$(get_rc_file "$shell_env" "$os")
